...
export class EisenhowerView extends ItemView {
  ...
  private renderQuadrant(parent: HTMLElement, key: QuadrantKey): void {
    ...
    const quadrantEl = parent.createDiv({
      cls: `eisenhower-quadrant eisenhower-${key}`,
    });
    ...
    const headerText = header.createDiv({
      cls: "eisenhower-quadrant-header-text",
    });
    headerText.createEl("div", {
      cls: "eisenhower-quadrant-title",
      text: labels.title,
    });
    headerText.createEl("div", {
      cls: "eisenhower-quadrant-subtitle",
    ...
    const expandBtn = header.createEl("button", {
      ...
      attr: {
        "aria-label":
          this.fullscreenQuadrant === key ? "Exit fullscreen" : "Fullscreen",
      },
    });
    setIcon(
      expandBtn,
      this.fullscreenQuadrant === key ? "minimize-2" : "maximize-2",
    );
    ...
    const sectionsContainer = quadrantEl.createDiv({
      cls: "eisenhower-sections-container",
    });
  ...
  private renderSection(
    ...
    const headerRow = sectionEl.createDiv({
      cls: "eisenhower-section-header",
    });
    const nameSpan = headerRow.createEl("span", {
      cls: "eisenhower-section-name",
      text: section.name,
    });
  ...
  private renderAddSectionRow(
    ...
    const submit = async () => {
      ...
      const exists = this.taskManager.data.sections[quadrant].some(
        (s) => s.name === name,
      );
      if (exists) return;
  ...
  private showFilePicker(targetInput: HTMLInputElement): void {
    ...
    new FileLinkModal(
      ...
      (file: TFile) => {
        ...
        const linkPath = file.path.endsWith(".md")
          ? file.path.slice(0, -3)
          : file.path;
